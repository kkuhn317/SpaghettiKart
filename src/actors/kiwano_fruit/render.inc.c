...
void render_actor_kiwano_fruit(UNUSED Camera* camera, Mat4 arg1, struct Actor* actor) {
    ...
    arg1[3][0] = actor->pos.x;
    arg1[3][1] = actor->pos.y;
    arg1[3][2] = actor->pos.z;
    ...
    // Animate actor by creating an index from rot.x. Divide by texture size to get whole number 0-2.
    // All three animated textures need to be the same size for this to work.
    size_t idx = (actor->rot.x << 0xA) / ResourceGetTexSizeByName(gTextureDksJungleParkwayKiwanoFruit1);
    switch(idx) {
        case 0:
    ...
    //addr = segmented_texture_to_virtual((actor->rot.x << 0xA) + 0x03009000);

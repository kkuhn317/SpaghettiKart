...
void render_actor_banana(Camera* camera, UNUSED Mat4 arg1, struct BananaActor* banana) {
    ...
    f32 temp = is_within_render_distance(camera->pos, banana->pos, camera->rot.y, 0, gCameraZoom[camera - camera1],
                                         490000.0f);
    ...
    if ((banana->pos.y > gCourseMaxY + 800.0f)) {
        actor_not_rendered(camera, (struct Actor*) banana);
        return;
    }
    if (banana->pos.y < (gCourseMinY - 800.0f)) {
        actor_not_rendered(camera, (struct Actor*) banana);
        return;
    }
